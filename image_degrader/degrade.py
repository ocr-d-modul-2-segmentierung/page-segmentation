...
def _make_noise_at_scale(shape, scale):
    h, w = shape
...
def make_multiscale_noise(shape, scales, weights=None, span=(0.0, 1.0)):
    ...
    for s, w in zip(scales, weights):
        result += _make_noise_at_scale(shape, s) * w
    lo, hi = span
...
def _make_fiber(l, a, stepsize=0.5):
...
def make_fibrous_image(shape, nfibers=300, l=300, a=0.2, stepsize=0.5, span=(0.1, 1.0), blur=1.0):
    ...
    for i in range(nfibers):
        ...
        fiber = _make_fiber(l, a, stepsize=stepsize)
